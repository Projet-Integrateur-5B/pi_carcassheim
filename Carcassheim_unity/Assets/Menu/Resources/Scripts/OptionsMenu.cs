...
public class OptionsMenu : Miscellaneous
{
	...
	void Start()
	{
		...
		//on enleve de ce tableau les Text avec le tag no_trad (mots qu'on a pas besoin de traduire) et modif (les textes modifiées par le script) et les textes vides
		all_textes = Array.FindAll(all_textes, i => i.tag != "no_trad" && i.tag != "modif" && !string.IsNullOrEmpty(i.text)).ToArray();

		//on convertit le tableau en liste (pour faciliter les opérations)
		l_all_textes = all_textes.ToList<Text>();

		/*foreach (Text a in l_all_textes)
			Debug.Log(a.text);*/

		//liste contenant tous les textes en français
		fr_textes = new List<string>();
		foreach (Text a in l_all_textes)
			fr_textes.Add(a.text);

		//liste contenant tous les textes en anglais (tres sensible à l'ordre des gameobject)
		...
		//liste contenant tous les textes en allemand (tres sensible à l'ordre des gameobject)
		de_textes = new List<string> {"STATUS", "Raum-ID -> X", "OPTIONEN", "Erweiterungen", "Jahr ...", "Beitritt nach ID", "CREDITS", "VERBINDEN", "Fluss", "Geben Sie Ihr Passwort ein ...", "Zeit pro Runde", "Geben Sie Ihren Benutzernamen ein ...", "ÖFFENTLICHER RAUM", "ANLEGEN", "RAUM ANMELDEN", "ID zum Betreten", "BEENDEN", "Abtei", "Der Raum wird erstellt!", "Geben Sie Ihr Passwort ein ...", "E-Mail / Benutzername", "Akzeptieren ", "Geben Sie Ihre Email/Pseudo ein ...", "Bestätigen Sie Ihr Passwort ...", "EIN KONTO ERSTELLEN", "Monat ...", "STATUS", "Tag ...", "Endgame", "Eine ID eingeben ...", "SOLO SPIELEN", "KENNUNG", "Max Spieler", "Kennwort", "Geben Sie Ihre E-Mail ein ...", "EIN KONTO ERSTELLEN", "Öffentlich", "HILFE", "STATISTIKEN", "Option A", "MULTI SPIELEN", "Passwort anzeigen", "ZIMMERMENÜ BETRETEN", "Passwort anzeigen", "Privat", "Erstellen Sie Ihr Konto", "Bestätigen Sie Ihr Passwort", "Raum-Einstellungen", "JOIN BY ID", "Bedingung zum Beenden", "Optionen", "Öffentlich", "GCU", "E-Mail", "Erstellen Sie Ihren Raum", "Haben Sie Ihren Benutzernamen/Passwort vergessen?", "Benutzername", "Raum ", "Anzahl der Spieler ", "Kennwort", "Privat", "ANMELDEN", "Geburtsdatum", "Gastgeber", "READY", "Spieler", "Kachel", "ID", "Spieler", "Gastgeber", "Endspiel", "Maximale Spieler" };
	...
	public void ToggleValueChangedOM(Toggle curT)
	{
		...
		if (curT.name == "Toggle French")
		{
			...
			//on met a jour les textes en français
