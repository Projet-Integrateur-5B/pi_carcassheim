...
public partial class Server
{
    ...
    public static void PlayerJoin(Packet packetReceived, ref Packet packet, Socket socket)
    {
        ...
        try
        {
            nouveauPort = gestionnaire.GetPortFromPartyID(Int32.Parse(packetReceived.Data[0]));
        }
        catch (Exception ex)
        {
            Console.WriteLine("ERROR: GetPortFromPartyID : " + ex);
        ...
        var playerStatus = PlayerJoinRoom(packetReceived.Data[0], packetReceived.IdPlayer, Socket socket);
        if (playerStatus == Tools.PlayerStatus.Success)
        {
            ...
            packet.Data = packet.Data.Concat(nouveauPort.ToString());
